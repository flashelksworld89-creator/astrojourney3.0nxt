...
import Digital3DMap from './Digital3DMap';
...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  return <div className={mapFullscreen?'mission-page map-is-fullscreen':'mission-page'}>
    ...
    {view==='map'&&<div className={mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'}>
      <div className="map-render-mode-switch"><button className={mapRenderMode==='map'?'active':''} onClick={()=>setMapRenderMode('map')}>2D Map</button><button className={mapRenderMode==='3d'?'active':''} onClick={()=>{setStreetViewActive(false);setMapRenderMode('3d')}}>Digital 3D</button></div>
      <div className={mapRenderMode==='3d'?'base-map-under-3d':''}><GoogleMissionMap location={liveLocation} analysisLocation={analysisLocation} destination={journeyDestination} fullscreen={mapFullscreen} radiusMeters={wheelRadiusMeters} followUser={trackingMode!=='static'} cityCentered cityCenter={cityCenter} cityRadiusMeters={geographicRadiusMeters} onCityCenterChange={setCityCenter} onStreetViewChange={setStreetViewActive} onStreetPovChange={setStreetPov} onCityViewportChange={g=>setCityWheelDiameterPx(Number(g?.diameterPx)||null)} onMapPlaceSelect={setMapSelectedPlace} focusLocation={mapFocusLocation} streetLocation={mapStreetLocation} digitalStreetMode={streetViewActive}/></div>
      <Digital3DMap active={mapRenderMode==='3d'&&!streetViewActive} location={liveLocation} destination={journeyDestination} cityCenter={cityCenter} cityRadiusMeters={geographicRadiusMeters} currentZone={geographicNakshatraZone||currentTravelZone} currentBearing={cityBearing} followedPlanet={followedPlanet} followedBearing={followedBearing}/>
      ...
      {streetViewActive&&<StreetGameOverlay active heading={streetPov.heading} currentZone={geographicNakshatraZone||currentTravelZone} currentBearing={cityBearing} followedPlanet={followedPlanet} followedBearing={followedBearing} followedZone={followedLineZone} onStopFollowing={()=>setFollowingPlanetId(null)}/>}
      ...
      {cityCenter&&!streetViewActive&&mapRenderMode!=='3d'&&<div className="city-center-hud"><small>Geographic wheel center</small><b>{cityCenter.label}</b><span>You are in geographic House {geographicHouse||'—'} · {Number.isFinite(cityDistanceMeters)?formatDistance(cityDistanceMeters/1000,distanceUnit):'—'} from center</span></div>}
      {chart&&mapRenderMode!=='3d'&&<div className={`map-wheel-overlay ${streetViewActive?`street-${streetCompassMode}`:''}`} style={{'--wheel-size':`${wheelDisplaySize}px`}}><div className="compass-overlay-label">{streetViewActive?'STREET SIDEREAL COMPASS':'LIVE SIDEREAL COMPASS'}</div><ZodiacWheel chart={chart} natalAsc={natalChart?.asc} natalMc={natalChart?.mc} natalPlanets={natalPlanets} natalBirthBearing={natalBirthBearing} planets={planets} selectedPlanet={selected} onSelectPlanet={choosePlanet} selectedHouse={selectedHouse} onSelectHouse={chooseHouse} destinationBearing={bearing} compact overlay radiusMeters={geographicRadiusMeters} distanceUnit={distanceUnit} displayHeading={streetViewActive?streetPov.heading:0} followingPlanetId={followingPlanetId} flatMode={flatModeActive} travelBearing={currentTravelBearing} cityCentered cityCenterLabel={cityCenter?.label} userGeoBearing={cityBearing} userGeoDistanceMeters={cityDistanceMeters} geoRadiusMeters={geographicRadiusMeters} geographicHouse={geographicHouse}/></div>}
    </div>}
