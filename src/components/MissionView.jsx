...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  return <div className={mapFullscreen?'mission-page map-is-fullscreen':'mission-page'}>
    ...
    {view==='map'&&<div className={mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'}>
      <div className="map-render-mode-switch"><button className={mapRenderMode==='map'&&!streetViewActive?'active':''} onClick={()=>{setMapRenderMode('map');setStreetViewActive(false)}}>2D Map</button><button className={streetViewActive?'active':''} onClick={openCurrentStreet}>Street View</button><button className={mapRenderMode==='3d'&&!streetViewActive?'active':''} onClick={()=>{setStreetViewActive(false);setMapRenderMode('3d')}}>Digital 3D</button></div>
      <div className={mapRenderMode==='3d'&&!streetViewActive?'base-map-under-3d':''}><GoogleMissionMap location={liveLocation} analysisLocation={analysisLocation} destination={journeyDestination} fullscreen={mapFullscreen} radiusMeters={wheelRadiusMeters} followUser={trackingMode!=='static'} cityCentered cityCenter={cityCenter} cityRadiusMeters={geographicRadiusMeters} onCityCenterChange={setCityCenter} onStreetViewChange={active=>{setStreetViewActive(active);if(active)setMapRenderMode('map')}} onStreetPovChange={setStreetPov} onCityViewportChange={g=>setCityWheelDiameterPx(Number(g?.diameterPx)||null)} onMapPlaceSelect={setMapSelectedPlace} focusLocation={mapFocusLocation} streetLocation={mapStreetLocation} digitalStreetMode={streetViewActive}/></div>
