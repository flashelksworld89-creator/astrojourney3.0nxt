...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  return <div className={mapFullscreen?'mission-page map-is-fullscreen':'mission-page'}>
    ...
    {view==='map'&&<div className={`${mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'} ${fullscreenSettling?'map-transitioning':''}`}>
      ...
      {mapFullscreen&&<div className={`fullscreen-prediction-drawer ${fullscreenPredictionOpen?'open':''}`}>
        <button type="button" className="fullscreen-prediction-tab" onClick={()=>setFullscreenPredictionOpen(v=>!v)} aria-expanded={fullscreenPredictionOpen}>
          <Sparkles size={15}/><span>Predictions</span>{(selected||selectedHouse)&&<b>{selected?`${selected.glyph} ${selected.name}`:`House ${selectedHouse}`}</b>}
        </button>
        {fullscreenPredictionOpen&&<div className="fullscreen-prediction-body">
          <div className="fullscreen-prediction-head"><div><small>Active forecast</small><strong>{focusTitle}</strong></div><button type="button" onClick={()=>setFullscreenPredictionOpen(false)}>×</button></div>
          {reading?.summary&&<div className="fullscreen-journey-summary"><small>Journey forecast</small><p>{reading.summary}</p></div>}
          <ForecastPanel forecast={focusForecast} fallback="Tap a planet glyph or numbered house on the compass to open its full prediction here."/>
