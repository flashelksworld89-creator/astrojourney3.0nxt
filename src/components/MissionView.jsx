...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  return <div className={mapFullscreen?'mission-page map-is-fullscreen':'mission-page'}>
    ...
    {view==='map'&&<div className={mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'}>
      <GoogleMissionMap location={liveLocation} analysisLocation={analysisLocation} destination={mission.destination} fullscreen={mapFullscreen} radiusMeters={wheelRadiusMeters} followUser={trackingMode!=='static'} cityCentered cityCenter={cityCenter} cityRadiusMeters={geographicRadiusMeters} onCityCenterChange={setCityCenter} onStreetViewChange={setStreetViewActive} onStreetPovChange={setStreetPov} onCityViewportChange={g=>setCityWheelDiameterPx(Number(g?.diameterPx)||null)}/>
      {streetViewActive&&<StreetGameOverlay active heading={streetPov.heading} currentZone={currentTravelZone} followedPlanet={followedPlanet} followedBearing={followedBearing} followedZone={followedLineZone} onStopFollowing={()=>setFollowingPlanetId(null)}/>}
