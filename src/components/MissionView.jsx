...
import MapDirectionControls from './MapDirectionControls';
...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  const changeTrackingMode=mode=>{
    ...
    if(mode==='static'){
      ...
      setLastAnalysisAt(Date.now());
      setMovedMeters(0);
      // Tracking mode no longer forces the wheel scale. The canonical 1-mile
      // nakshatra diameter remains available while walking/driving presets can be chosen manually.
    }
  };
  ...
  return <div className={mapFullscreen?'mission-page map-is-fullscreen':'mission-page'}>
    ...
    {view==='map'&&<div className={mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'}><GoogleMissionMap location={liveLocation} analysisLocation={analysisLocation} destination={mission.destination} fullscreen={mapFullscreen} radiusMeters={wheelRadiusMeters} followUser={trackingMode!=='static'} onOverlayMetrics={setMapOverlayMetrics}/><MapScaleControls radiusMeters={wheelRadiusMeters} unit={distanceUnit} onUnitChange={setDistanceUnit} onRadiusChange={setWheelRadiusMeters}/><MapDirectionControls bearing={travelBearing ?? bearing} routeBearing={bearing} onChange={setTravelBearing}/><div className="map-tracking-overlay"><LiveTrackingControls mode={trackingMode} onModeChange={changeTrackingMode} gps={gps} gpsError={gpsError} movedMeters={movedMeters} lastAnalysisAt={lastAnalysisAt}/></div><button className="fullscreen-button" onClick={()=>setMapFullscreen(v=>!v)}>{mapFullscreen?<Minimize2 size={17}/>:<Maximize2 size={17}/>} {mapFullscreen?'Exit full screen':'Full screen'}</button>{chart&&<div className="map-wheel-overlay dynamic-wheel" style={{'--wheel-size':`${(mapOverlayMetrics.diameterPx||520)*1.18}px`,'--wheel-left':Number.isFinite(mapOverlayMetrics.userX)?`${mapOverlayMetrics.userX}px`:'50%','--wheel-top':Number.isFinite(mapOverlayMetrics.userY)?`${mapOverlayMetrics.userY}px`:'50%'}}><div className="compass-overlay-label">LIVE SIDEREAL COMPASS · 1 MI NAKSHATRA DIAMETER</div><ZodiacWheel chart={chart} natalAsc={natalChart?.asc} planets={planets} selectedPlanet={selected} onSelectPlanet={choosePlanet} selectedHouse={selectedHouse} onSelectHouse={chooseHouse} destinationBearing={bearing} travelBearing={travelBearing ?? bearing} compact overlay radiusMeters={wheelRadiusMeters} distanceUnit={distanceUnit}/></div>}</div>}
