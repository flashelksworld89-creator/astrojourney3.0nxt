  ...
  return <div className={mapFullscreen?'mission-page map-is-fullscreen':'mission-page'}>
    ...
    {view==='map'&&<div className={mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'}>
      ...
      {flatModeActive&&<div className="flat-travel-hud">
        ...
        <div><small>Nakshatra</small><b>{followedNak?.name||'—'}{followedNak?.pada?` · Pada ${followedNak.pada}`:''}</b></div>
      ...
      {chart&&<div className={`map-wheel-overlay ${streetViewActive?`street-${streetCompassMode}`:''}`} style={{'--wheel-size':`${wheelDisplaySize}px`}}><div className="compass-overlay-label">{streetViewActive?'STREET SIDEREAL COMPASS':'LIVE SIDEREAL COMPASS'}</div><ZodiacWheel chart={chart} natalAsc={natalChart?.asc} planets={planets} selectedPlanet={selected} onSelectPlanet={choosePlanet} selectedHouse={selectedHouse} onSelectHouse={chooseHouse} destinationBearing={bearing} compact overlay radiusMeters={wheelRadiusMeters} distanceUnit={distanceUnit} displayHeading={streetViewActive?streetPov.heading:0} followingPlanetId={followingPlanetId} flatMode={flatModeActive} travelBearing={currentTravelBearing}/></div>}
    </div>}
