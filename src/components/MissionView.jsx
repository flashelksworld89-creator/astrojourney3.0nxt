...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  useEffect(()=>{if(!chart||!natalChart||!baseReading)return;let cancelled=false;setPrivateReadingBusy(true);requestPrivateInterpretation({chart,natalChart,houseLords,origin:analysisLocation,destination:journeyDestination,bearing:baseReading.bearing,direction:baseReading.direction,distanceKm:baseReading.distanceKm,selectedDate:date,focusHouses,focusLords,relocationCurrent,relocationDestination,localSpaceContacts}).then(r=>!cancelled&&setPrivateReading(r)).catch(()=>!cancelled&&setPrivateReading(null)).finally(()=>!cancelled&&setPrivateReadingBusy(false));return()=>{cancelled=true}},[chart,natalChart,baseReading,houseLords,analysisLocation,journeyDestination,date,focusHouses,focusLords,relocationCurrent,relocationDestination,localSpaceContacts]);
