...
export default function GoogleMissionMap({location,analysisLocation,destination,fullscreen,radiusMeters=152.4,followUser=true}) {
  ...
  useEffect(()=>{const circle=scaleCircleRef.current;if(!circle)return;circle.setRadius(Math.max(15,Number(radiusMeters)||804.672));},[radiusMeters]);

  useEffect(()=>{const map=mapRef.current;if(!map)return;const center=map.getCenter?.();const zoom=map.getZoom?.();const timers=[0,80,220,500].map(delay=>setTimeout(()=>{window.google?.maps?.event?.trigger(map,'resize');if(center)map.setCenter(center);if(Number.isFinite(zoom))map.setZoom(zoom);if(followUser&&location)map.panTo(location);},delay));return()=>timers.forEach(clearTimeout)},[fullscreen]);
